...
def intent_create(request,botId):
        try:
            ...
            try:
                example = json.dumps(intentOb.get("example"),ensure_ascii=False)
...
def utterance_create(request,botId):
    try:
        ...
        try:
            example = json.dumps(utteranceOb.get("example"), ensure_ascii=False)
            utterance.example = example
        except:
...
def story_create(request,botId):
    ...
    try:
        example = json.dumps(storiesOb.get("example"), ensure_ascii=False)
        stories.example = example
    except:
...
def slot_create(request,botId):
    ...
    slot_type = json.dumps(slotsOb['slot_type'], ensure_ascii=False)
    slot.slot_type = slot_type
    slot.slot_type = slotsOb['slot_type']
    for item in slotsOb['mapping']:
        #验证映射类型
        if item["type"] == "实体":
            try:
                item["entity"]
    ...
    mapping = json.dumps(slotsOb['mapping'], ensure_ascii=False)
...
def form_create(request,botId):
    ...
    name = form_info_data.get("name")
    existing_record = FormInfo.objects.filter(name=name, bot_id=botId).first()
    if existing_record:
        return Response({"message": "此表单名已存在"}, status=500)
        ...
        if form_info_serializer.is_valid():
            ...
            try:
                ...
                for item in form_slot_info_data:
                    ...
                    slot = Slot.objects.get(name=slot_name,bot_id=botId)
                    ...
                    form_slot_info.question = question
                    ...
                    if form_slot_info_serializer.is_valid():
                        ...
                        try:
                            utterance_ask = Utterance.objects.get(name=utter_ask_name, bot_id=bot_id)
                            utterance_ask.example = question
                            utterance_ask.save()
                        except Utterance.DoesNotExist:
                            utterance_ask = Utterance.objects.create(name=utter_ask_name, example=question,
                                                                     bot_id=bot_id)
                        ...
                        try:
                            utterance_valid = Utterance.objects.get(name=utter_valid_name, bot_id=bot_id)
                            utterance_valid.example = valid
                            utterance_valid.save()
                        except Utterance.DoesNotExist:
                            utterance_valid = Utterance.objects.create(name=utter_valid_name, example=valid,
                                                                       bot_id=bot_id)

                        try:
                            utterance_invalid = Utterance.objects.get(name=utter_invalid_name, bot_id=bot_id)
                            utterance_invalid.example = invalid
                            utterance_invalid.save()
                        except Utterance.DoesNotExist:
                            utterance_invalid = Utterance.objects.create(name=utter_invalid_name, example=invalid,
                                                                         bot_id=bot_id)
                    else:
...
def form_update(request, botId, formId):
    ...
    try:
        ...
        for item in form_slot_info_data:
            ...
            #判断一下数据库中是否有这个槽详细信息
            if not form_info_instance.slots.filter(pk=slot.pk).exists():
                form_info_instance.slots.add(slot)  # 建立多对多关系
                ...
                if form_slot_info_serializer.is_valid():
                    ...
                    utterance_ask, created = Utterance.objects.update_or_create(name=utter_ask_name, example=question,
                                                                                bot_id=bot_id)
                    utterance_ask.save()
                    ...
                    utterance_invalid, created = Utterance.objects.update_or_create(name=utter_invalid_name,
                                                                                    example=invalid,
                                                                                    bot_id=bot_id)
                    utterance_invalid.save()
                else:
