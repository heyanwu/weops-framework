...
from haystack.constants import DJANGO_CT, DJANGO_ID, FUZZY_WHOOSH_MAX_EDITS, FUZZY_WHOOSH_MIN_PREFIX, ID
...
from utils.app_log import logger as weops_log
...
DATETIME_REGEX = re.compile(
    r"^(?P<year>\d{4})-(?P<month>\d{2})-(?P<day>\d{2})T(?P<hour>\d{2}):(?P<minute>\d{2}):(?P<second>\d{2})(\.\d{3,6}Z?)?$"  # noqa
)
...
class WhooshSearchBackend(BaseSearchBackend):
    ...
    def update(self, index, iterable, commit=True):
        ...
        if len(iterable) > 0:
            ...
            weops_log.info("==索引写入提交开始==")
            _return = False
            try:
                writer.commit()
                _return = False
            except BaseException as e:
                weops_log.exception("索引写入出错：{}".format(e))
                writer.writer._finish()
                _return = True
                weops_log.exception("手动释放线程锁完成")

            weops_log.info("==索引写入提交结束==")
            weops_log.info("writer.ident={}".format(writer.ident))
            if writer.ident is not None:
                weops_log.info("==索引写入提交结束,正在等待线程结束==")
                writer.join()
                weops_log.info("==索引写入提交结束,线程结束==")
    ...
    def remove(self, obj_or_string, commit=True):
        ...
        try:
            weops_log.info("==索引删除开始==")
            self.index.delete_by_query(q=self.parser.parse('{}:"{}"'.format(ID, whoosh_id)))
            weops_log.info("==索引删除结束==")
        except Exception as e:
            if not self.silently_fail:
                raise
    ...
    def clear(self, models=None, commit=True):
        ...
        try:
            if models is None:
                ...
                for model in models:
                    models_to_delete.append("{}:{}".format(DJANGO_CT, get_model_ct(model)))
    ...
    def search(  # noqa
        ...
        **kwargs
        ...
        if facets is not None:
            group_by += [FieldFacet(facet, allow_overlap=True, maptype=Count) for facet in facets]
        ...
        if len(model_choices) > 0:
            ...
            narrow_queries.add(" OR ".join(["{}:{}".format(DJANGO_CT, rm) for rm in model_choices]))
    ...
    def more_like_this(
        ...
        **kwargs
        ...
        searcher = None
        if self.index.doc_count():
            query = "{}:{}".format(ID, get_identifier(model_instance))
            searcher = self.index.searcher()
    ...
    def _process_results(
        ...
        if facet_types:
            ...
            for facet_fieldname in raw_page.results.facet_names():
                ...
                lst = facets[facet_type][facet_fieldname] = sorted(group.items(), key=(lambda itm: (-itm[1], itm[0])))
...
class WhooshSearchQuery(BaseSearchQuery):
    ...
    def build_query_fragment(self, field, filter_type, value):  # noqa
        from haystack import connections
        ...
        if value.post_process is False:
            ...
            if filter_type in [
                    ...
                    for possible_value in possible_values:
                        ...
                        if filter_type == "fuzzy":
                            terms.append(
                                ...
                                % (
                                    possible_value_str,
                                    min(FUZZY_WHOOSH_MIN_PREFIX, len(possible_value_str)),
                                )
                            )
                        else:
                ...
                query_frag = "[{} to {}]".format(start, end)
        ...
        return "{}{}".format(index_fieldname, query_frag)
